...
class KalmanFilter(Node):
    def __init__(self):
        ...
        self.odom_sub_ = self.create_subscription(Odometry, "fenrir_controller/odom_noisy", self.odomCallback, 10)
        self.imu_sub_ = self.create_subscription(Imu, "imu", self.imuCallback, 10)

        self.odom_pub = self.create_publisher(Odometry, "fenrir_controller/odom_kalman", 10)
