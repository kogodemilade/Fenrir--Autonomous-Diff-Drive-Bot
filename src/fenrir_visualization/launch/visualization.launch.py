...
def generate_launch_description():
    ...
    pkg_share = get_package_share_directory('fenrir_visualization')
    config_path = os.path.join(pkg_share, 'config', 'image_bridge.yaml')

    # set_env_vars = SetEnvironmentVariable(name='PYTHONPATH', 
    #                                       value="$HOME/fenrir_ws/.venv/lib/python3.12/site-packages:$PYTHONPATH")
    # Bridge camera topics

    camera_stream = Node(        
        package="fenrir_visualization",
        ...
        'PYTHONPATH':
            [EnvironmentVariable('HOME'),
             TextSubstitution(text='/fenrir_ws/.venv/lib/python3.12/site-packages:'),
