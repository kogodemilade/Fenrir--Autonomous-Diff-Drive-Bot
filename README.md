...
![Fenrir](images/diff_drive_with_map.png)
...
Each package serves a distinct purpose, which is identifiable by its name. Each package is explained below. For Installation instructions, [Click here](#installation) (or find below).
#

<a name=packages></a>
### Packages
**Robot description (fenrir_description)**
...
**Local Localization (fenrir_localization)**:
...
**Visualization (fenrir_visualization)**:
This package handles the vision model, YOLO11, which runs object detection (no segmentation for now). Currently, I'm thinking of creating another model through transfer learning from the yolo model for better detection of objects more likely to be seen by Fenrir, or upgrading to the m/l yolo models.

**Sensor Reframing and Bridging (fenrir_bridges)**:
...
**SLAM (fenrir_slam)**:
SLAM (Simultaneous Localization and Mapping) refers to 2 things. The first is Mapping. That is, building a map of the environment from sensor scans. Lidars are typically the primary sensors used for this task, but rgbd (d for depth) camera sensors are also used. In this project, rgbd cameras are employed. Localization refers to estimating where the robot is with the map being built as a reference. For example, imagine Fenrir was dropped in a bedroom. Mapping answers the question 'What is the geometry of this room (including objects)?' while localization answers 'Where am I within this geometry?'.
...
**Navigation (fenrir_nav)**:
This takes the map from the slam package and actually navigates through the map when given a task (or 'Goal'). It uses Behaviour Trees (BTs) as well as other sensor data for decision-making. It uses Ros2's 'Nav2' package internally. Currently still in development.
#
...
cd <workspace_name>
#Replace <workspace_name> with a name of your choice. Eg. fenrir_ws
```
#
...
#source current project (fenrir_ws, replace with your directory file path)
...
```bash 
ros2 launch fenrir_<pkg> app.launch.py
```
...
```bash
ros2 launch fenrir_description gazebo.launch.py
#New tab (Ctrl+Shift+E on terminator)
ros2 launch fenrir_controller controller.launch.py
#New tab 
ros2 launch fenrir_description display.launch.py
#New tab
ros2 launch fenrir_localization local_localization.launch.py
```
